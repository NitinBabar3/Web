...
class Utilities {
    ...
    static function generateHashedPassword($password, $salt) {
        include 'Hash.php';
	...
	static function RegisterSession($obj)
	{
			...
			if($UserId!="")
		    {
			   ...
			   {
				$session_array=array("Email"=>$obj->Email,"UserId"=>$User_id, "UserTypeId"=>$UserTypeId);
			   }
			   else
			   {
			    $session_array=array("Email"=>$obj->Email,"UserId"=>$User_id, "UserTypeId"=>$UserTypeId,"ApplicationId"=>$obj->ApplicationId);
			   }
	...
	static function RegisterStudentSession($obj)
	{
		session_start();
		$session_array=array("FacebookId"=>$obj);
		$_SESSION['letssession']=$session_array;
	}
	static function UpdateSession($obj,$applicationID)
	{
		session_start();
		$session_array=array("Email"=>$obj->Email,"UserId"=>$obj->User_id, "UserTypeId"=>$obj->UserTypeId,"ApplicationID"=>$applicationID);
		$_SESSION['letssession']=$session_array;
	}
    ...
    function GetJobCardObjectWithIdStringsReplacedByAttributes($objJobCardArray){
	 ...
	 static function FBUserExists($id){
        try{
				$query="SELECT Email FROM student where FacebookID='$id'";
				//echo $query;
	...
	static function GetStudentCard($obj){
       try{
				...
				$design= "<table>
							<tr>
							<td><img src='http://graph.facebook.com/".$obj."/picture?type=large'  height='130' width='140' border='0'/>
							</td>
							<td>
							<table>
							<tr>
	...
	static function GetHTMLDropdownFromListwithCondition($name_of_select,$condition){
	 ...
	 static function GetStudentInfo($id){
        ...
        $query="SELECT CollegeId,CurrentYear,CurrentCourseId,LocationId,ProfileTitle,FacebookID,Email,Picture,MarksXth,MarksXIIth from student WHERE FacebookId=$id";
        $getlist= MySQLDataAdapter::Query($query);
	    $row = mysql_fetch_array($getlist);	
		return  $row;
    }
	static function GetStudentCollegeInfo($id){
        
        $query="SELECT CollegeName from college WHERE ID=$id";
		//echo  $query;
        $getlist= MySQLDataAdapter::Query($query);
	    $row = mysql_fetch_array($getlist);	
		return  $row;
    }
	static function GetStudentSkills($id){
        
        $query="SELECT SkillName FROM skills
    			INNER JOIN student_skills ON
				student_skills.SkillId= skills.ID
				WHERE student_skills.StudentId=$id";
		//echo  $query;
        $getlist= MySQLDataAdapter::Query($query);
	    $row = mysql_fetch_array($getlist);	
		return  $row;
    }
	static function GetStudentWorkExperience($id){
        
        $query="SELECT student_workex.CompanyName,location.CityName from student_workex
				INNER JOIN location
				ON location.ID=student_workex.LocationID
				WHERE student_workex.StudentId='".$id."'";
		//echo  $query;
        $getlist= MySQLDataAdapter::Query($query);
	    $row = mysql_fetch_array($getlist);	
		return  $row;
    }
	static function GetFollowedCompanyInfo($id){
        ...
        $query="SELECT employer.CompanyName,followers.employer_id,followers.StartDate from followers
				...
				WHERE followers.student_id='".$id."'";
		//echo  $query;
        $getlist= MySQLDataAdapter::Query($query);
	    return  $getlist;
    }
	
	static function LogActivity($obj,$debug_level){
        
		if($debug_level=="1")
		{
			$query="INSERT into activity_log(CodeID,UserID,Feature,CreatedOn) VALUES
			('".$obj->CodeID."','".$obj->UserId."','".$obj->Feature."','".$obj->CreatedOn."')";
			//echo  $query;
			$getlist= MySQLDataAdapter::Query($query);
	    } 
		else {
	...
	static function GetEmployerJobs(){
       try{
				
				
				$query="SELECT location.CityName,industry.IndustryType,employer.* FROM employer 
				        INNER JOIN location ON
						location.ID=employer.LocationIDString
						INNER JOIN industry ON
						industry.ID=employer.IndustryID
						ORDER BY employer.UpdatedOn";
				//echo $query;
	...
	function CheckloginBetatester($email,$password)
	{
		 $query="SELECT ID FROM betatester WHERE Email='".$email."' AND Password='".$password."'";
		//echo  $query;
        $getlist= MySQLDataAdapter::Query($query);
	    $row = mysql_fetch_array($getlist);	
	...
	function LogBetatesterLogin($TesterID)
	{
		$query="INSERT INTO betatesteractivity(ID,LastLogin) VALUES ('".$TesterID."','".date("Y-m-d h:i:s")."')";
		//echo  $query;
        $getlist= MySQLDataAdapter::Query($query);
	    $row = mysql_fetch_array($getlist);	
		$num = mysql_num_rows($getlist);	
	...
	function StudentWorkExperience($obj)
	{
		$query="INSERT INTO student_workex (StudentId,CompanyName,Title,LocationID,JobTypeID,UpdatedOn) VALUES 
		('".$obj->StudentId."',
		'".$obj->CompanyName."',
		'".$obj->Title."',
		'".$obj->LocationID."',
	...
	function CheckAdminMaster($id)
	{
		$query="SELECT FacebookID FROM student WHERE Email='".$id."'";
		//echo  $query;
        $getlist= MySQLDataAdapter::Query($query);
	    $row = mysql_fetch_array($getlist);	
		return $row[0];
	}
	static function GetAdminHTMLDropdownFromList($list,$name_of_select){
        //$list contains the list of elements pointed by their ID.
        //It will convert that array into an HTML DropDown List.
        ...
        try{
            ...
            foreach($list as $id => $param_value)
            {
                $substring = "<option value =".$param_value.">".$param_value."</option>";
                $string = $string.$substring;
            }
            $string = $string."</select>";
        }
	...
	function CheckTestDetails($id)
	{
		$query="SELECT test.TestTitle,test.Duration,job.ScreeningTestID from test
		        INNER JOIN job ON
				job.ScreeningTestID=test.ID
				WHERE job.ID='".$id."'";
		//echo  $query;
        $getlist= MySQLDataAdapter::Query($query);
	    $row = mysql_fetch_row($getlist);	
	...
	$query="SELECT COUNT(*) FROM question
			INNER JOIN test_questions ON 
			test_questions.QuestionID=question.ID
			WHERE test_questions.TestId='".$TestID."' AND question.QuestionType='Audio'";
	//echo $query;
	$getlist= MySQLDataAdapter::Query($query);
	$row = mysql_fetch_row($getlist);	
	...
	{
		$query="SELECT * FROM alerts WHERE Reviewed='0' AND Deleted='0' AND StudentId='".$Id."'";
		//echo  $query;
        $getlist= MySQLDataAdapter::Query($query);
	    $row = mysql_fetch_row($getlist);	
		return $row;
	...
	{
		$query="INSERT INTO alert (StudentId,AlertTypeId,CreatedOn) 
		values ('".$obj->StudentId."','".$obj->AlertTypeId."','".Date("Y-m-d h:i:s")."')";
		//echo  $query;
        $getlist= MySQLDataAdapter::Query($query);
	    $row = mysql_fetch_row($getlist);	
		return $row;
